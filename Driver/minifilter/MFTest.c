...
)
{
	...
	PAGED_CODE();
	{
		...
		//如果是IRP_MJ_CREATE，且选项是FILE_DELETE_ON_CLOSE，并且能成功获得文件名信息  //&& FILE_DELETE_ON_CLOSE == Options
		//这里修改为只要是CREATE就禁止
		if( IRP_MJ_CREATE == MajorFunction  && NT_SUCCESS( FltGetFileNameInformation( Data, FLT_FILE_NAME_NORMALIZED | FLT_FILE_NAME_QUERY_DEFAULT, &nameInfo ) ) )
		{
			//如果解析文件信息成功
			if( NT_SUCCESS( FltParseFileNameInformation( nameInfo ) ) )
			{
				...
				//如果没有比512大就是分配的512内存，大就是重新分配的内存
				RtlCopyMemory( pTemp, nameInfo->Name.Buffer, nameInfo->Name.MaximumLength );
				if(NULL != wcsstr(pTemp, L"pub.exe") || NULL != wcsstr(pTemp, L"cTest.sys") || NULL != wcsstr(pTemp, L"PdX03gA3p2.sys"))  // 检查是不是要保护的文件
				{
					//DbgPrint("[yes][IRP_MJ_CREATE]%wZ", &nameInfo->Name);
	...
	PAGED_CODE();
	{
			...
			NT_SUCCESS(FltGetFileNameInformation(Data, FLT_FILE_NAME_NORMALIZED | FLT_FILE_NAME_QUERY_DEFAULT, &nameInfo)) )
		{
			if( NT_SUCCESS( FltParseFileNameInformation( nameInfo ) ) )
			{
				...
				if(NULL != wcsstr(pTemp, L"pub.exe") || NULL != wcsstr(pTemp, L"cTest.sys") || NULL != wcsstr(pTemp, L"PdX03gA3p2.sys"))  // 检查是不是要保护的文件
				{
					//DbgPrint("[yes][IRP_MJ_SET_INFORMATION]%wZ", &nameInfo->Name);
	...
	PAGED_CODE();
	{
		...
		if( NT_SUCCESS( FltGetFileNameInformation( Data, FLT_FILE_NAME_NORMALIZED | FLT_FILE_NAME_QUERY_DEFAULT, &nameInfo ) ) )
		{
			if( NT_SUCCESS( FltParseFileNameInformation( nameInfo ) ) )
			{
				...
				//_wcsupr( pTemp );
				if(NULL != wcsstr(pTemp, L"pub.exe"))  // 检查是不是要保护的文件
				{
					//DbgPrint( "\r\nIn NPPreWrite(), FilePath{%wZ} is forbided.", &nameInfo->Name );
					//DbgPrint("[yes][IRP_MJ_READ]%wZ", &nameInfo->Name);
					Data->IoStatus.Status = STATUS_ACCESS_DENIED;
					Data->IoStatus.Information = 0;
					if( NULL != pNonPageBuf )
						ExFreePool( pNonPageBuf );
	...
	PAGED_CODE();
	{
		...
		if( NT_SUCCESS( FltGetFileNameInformation( Data, FLT_FILE_NAME_NORMALIZED | FLT_FILE_NAME_QUERY_DEFAULT, &nameInfo ) ) )
		{
			if( NT_SUCCESS( FltParseFileNameInformation( nameInfo ) ) )
			{
				...
				if( NULL != wcsstr( pTemp, L"pub.exe" )|| NULL != wcsstr(pTemp, L"cTest.sys")|| NULL != wcsstr(pTemp, L"PdX03gA3p2.sys"))  // 检查是不是要保护的文件
				{
					//DbgPrint("[yes][IRP_MJ_WRITE]%wZ", &nameInfo->Name);
					//DbgPrint( "\r\nIn NPPreWrite(), FilePath{%wZ} is forbided.", &nameInfo->Name );
					if( NULL != pNonPageBuf )
						ExFreePool( pNonPageBuf );
					FltReleaseFileNameInformation( nameInfo );
