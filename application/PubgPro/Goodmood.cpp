...
RECT RRc;
const uint64_t Blank_Add = 0x6C91E00;
...
void hookGnames() {
	...
	DWORD_PTR NameAddress = mem->GetBase()+ 0x2F1F75;
...
unsigned char CameraManagerShellcode[] = {
	0x48,0x89,0x0D,0x00,0x00,0x00,0x00, //rcx
...
void hookCameraManager() {
	DWORD_PTR CameraAddress = mem->GetBase() + 0x3A575A4;
	...
	*(uint32_t *)(CameraManagerShellcode + 8) = (uint32_t)(mem->GetBase() + 0x2ECBBC0 - (JmpAddr + 7) - 5);
...
void hookActor() {
	DWORD_PTR ActorAddress = mem->GetBase() + 0x726872;
	...
	if (Actor::count > 0 && Actor::count < 3000) {
		...
		for (int i = 0; i < Actor::count; i++)
		{
			...
			if (id == ActorIds[0] || id == ActorIds[1] || id == ActorIds[2] || id == ActorIds[3]) {
				...
				if (health > 0.f)
				{
					...
					if (global::bPlayer) {
						...
						//if (id == ActorIds[0] || id == ActorIds[2])
						Actor::sex = Sex::famale;
						//else
						//	Actor::sex = Sex::male;
						DrawSkeleton(mesh, static_cast<Sex>(Actor::sex));
			...
			if (global::bLoot && (id == itemtype[0] || id == itemtype[1])) {
				...
				for (int j = 0; j < count; j++)
				{
					...
					logfile << GetNameFromId(ItemId) << "     " << ItemId;
					//if ((iter = Item.find(ItemId)) != Item.end()) {
					//	//Vector3 ItemAbPos = mem->RPM<Vector3>(pADroppedItemGroup + ItemAbsulotePos);

					//	Vector3 relative = mem->RPM<Vector3>(pADroppedItemGroup + ItemRelativeLocation); //RelativeLocation
					//	Vector3 pos = relative + GetActorPos(entity);
					//	Vector3 screenloc = WorldToScreen(pos, global::cameracache);
					//	DrawString((int)screenloc.x, (int)screenloc.y, iter->second.second, pFont2, "%s\n%d", Encrypt(iter->second.first.c_str()), (int)(local.Distance(pos) / 100.f));
					//}
				}
				continue;
			}
		...
		if (Aim::target&&global::bAimbot) {
			...
			if ((GetAsyncKeyState(VK_RBUTTON) & 0x8000) && Aim::AimPlayer != 0) {
				...
				int Weapon_Index = mem->RPM<uint8_t>(Weapon_Processor + WeaponBase + offset_weapon_index);//CurrentWeaponIndex
