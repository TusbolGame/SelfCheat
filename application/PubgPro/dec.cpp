...
uint32_t tsl_decrypt_id(const int& id) {
	int v4 = rol4(id ^ 0xD216FE25, 0x3);
	int decid = (v4 << 16)  ^0x9263CD63 ^ v4;
	return decid;
}
...
uint64_t tsl_decrypt_component(const uint64_t& Actor) {
	uint64_t ret = 0;
	uint64_t v5 = 0;
	uint64_t v6 = mem->RPM<DWORD_PTR>(Actor + RootComp);
	IDA_LODWORD(v5) = ror4(v6 ^ 0x493303BD, 16);
	IDA_LODWORD(v6) = ror4(IDA_HIDWORD(v6) ^ 0xAE62C6E8, 8);
	IDA_LODWORD(v5) = ror4((v5 - 463153956) ^ 0xE43303BD, 16);
	IDA_LODWORD(v6) = ror4((v6 - 480517284) ^ 0x4962C6E8, 8);
	IDA_LODWORD(ret) = v5 ^ 0xE464D4DC;
	IDA_HIDWORD(ret) = v6 ^ 0x1CA41CA4;
	return ret;
}
